...
- `se.kjellstrand.lsystemcamera` is the Android layer.
  ...
  - `MainScreen.kt` is the whole UI: Material 3 `Scaffold`, square output `Image`, `ExposedDropdownMenuBox` system picker with the PNG icons in `res/drawable`, three sliders. Permission rationale with "Allow camera" and "Open settings" buttons replaces the image until granted. `Theme.kt` applies dynamic color on API 31+ and the M3 baseline below that.
...
- Picker icons live in `res/drawable-nodpi` (they are drawn at a fixed dp size). The adaptive launcher icon reuses the foreground PNG as its monochrome layer.
